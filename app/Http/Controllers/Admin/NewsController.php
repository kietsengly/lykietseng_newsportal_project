...
class NewsController extends Controller
{
    ...
    /**
     * Generate a random slug (works with all languages).
     */
    private function generateRandomSlug($prefix = 'news')
    {
        return $prefix . '-' . Str::random(18) . '-' . time();
    }
    ...
    public function store(AdminNewsCreateRequest $request)
    {
        ...
        // ✅ use random slug
        $news->slug = $this->generateRandomSlug('news');
    ...
    public function update(AdminNewsUpdateRequest $request, string $id)
    {
        ...
        $news->title = $request->title;
        // ✅ regenerate slug only if explicitly requested (to keep old slug if not)
        if ($request->has('regenerate_slug') && $request->regenerate_slug == 1) {
            $news->slug = $this->generateRandomSlug('news');
        }
        $news->content = $request->content;
        $news->meta_title = $request->meta_title;
    ...
    public function copyNews(string $id)
    {
        ...
        $copyNews->slug = $this->generateRandomSlug('news'); // ✅ ensure copied news also has unique slug
        $copyNews->save();
